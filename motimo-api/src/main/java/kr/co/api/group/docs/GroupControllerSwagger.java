...
public interface GroupControllerSwagger {
    ...
    CustomSlice<GroupMessageItemRs> getGroupChat(UUID groupId, @RequestParam int page, @RequestParam int size);

    @Operation(summary = "그룹 리액션 API", description = "그룹 리액션을 생성합니다.")
    GroupMessageIdRs createGroupReaction(UUID userId, @PathVariable UUID messageId, @RequestParam ReactionType reactionType);
